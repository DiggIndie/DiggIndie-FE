...
export default function ArticleBody({ content }: ArticleBodyProps) {
  ...
  return (
    <div className="flex flex-col w-full py-4 px-5 border-b-4 border-gray-850">
      <span className="font-semibold text-xl text-white mb-1">{content?.title}</span>
      <p className="flex gap-[7px] mb-2">
        <span className="text-gray-600 font-medium text-sm">{content?.writerNickname}</span>
        <span className="text-gray-600 font-medium text-sm">{content?.createdAt}</span>
      </p>
      <div className="text-sm text-gray-300 font-normal mb-5 h-25">{content?.content}</div>
